...
int main(int argc, char *argv[])
{
    ...
    case '1':{
        ...
        //??????? is dis ok????
        while((bytesRead = read(0, buffer, BUFFER_SIZE))>0 && buffer[strlen(buffer)]!='\0'){ //ah shit sho ako e pred toa???????? ->strlen will dis work?
            ...
            if(write(1,buffer,bytesRead)==-1){
                printf("failed to zapise na ekran (how ironic)");
                exit(1);
            }
        }
        ...
        if(buffer[bytesRead-1]=="\0" && bytesRead>0){
            ...
            if(write(fd,buffer,bytesRead)==-1){
                printf("failed da zapise poslednoto na ekran");
                exit(1);
            }
        }
